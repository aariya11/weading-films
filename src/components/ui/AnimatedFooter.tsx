...
export function AnimatedFooter({
  headingLines = ["WEDDING FILMS", "STUDIO"],
  ...
  useEffect(() => {
    ...
    const setupHand = (
      ...
      const baselineOffset = effectiveCellSize / 2 + glyphHeight / 2 - metrics.actualBoundingBoxDescent;
    ...
    const renderHand = (hand: Hand, now: number) => {
      ...
      const { charColor: curCharColor, hoverColor: curHoverColor, hoverCharColor: curHoverCharColor } = liveRef.current;
      ...
      for (const cell of cellList) {
        ...
        if (isHighlighted) {
          ctx.fillStyle = curHoverColor;
          ctx.fillRect(x, y, cs, cs);
        }
        ctx.fillStyle = isHighlighted ? curHoverCharColor : curCharColor;
    ...
    const onTouchMove = (event: TouchEvent) => {
      ...
      const w = rect.width || 1;
      const h = rect.height || 1;
      pointer.x = ((touch.clientX - rect.left) / w - 0.5) * strength * 2;
      pointer.y = ((touch.clientY - rect.top) / h - 0.5) * strength * 2;
      for (const hand of hands) hoverHand(hand, touch.clientX, touch.clientY);
    };
  ...
  return (
    <div
      ref={rootRef as any}
      role="region"
      aria-label="Interactive studio signatures"
      className={cn(
        "relative h-full w-full overflow-hidden",
        !background && "bg-ink text-white",
        className
      )}
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-between">
        <div
          ref={leftWrapRef}
          className="relative w-2/5 min-w-[160px] sm:min-w-[200px] will-change-transform"
      ...
      {/* Display headings */}
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-center gap-2 sm:gap-4 p-4 sm:p-8">
        {headingLines.map((word, wi) => (
          <h2
            key={`${word}-${wi}`}
            aria-label={word}
            className="overflow-hidden font-medium leading-none tracking-tight pb-[0.15em] -mb-[0.15em] text-white"
            style={{ fontSize: "clamp(1.75rem, 12cqw, 10rem)" }}
          >
